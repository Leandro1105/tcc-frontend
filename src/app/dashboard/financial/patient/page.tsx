...
export default function PatientPaymentsPage() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const paymentsData = (await api.get(
          `/financeiro/paciente/${profile.id}`
        )) as Payment[];
  ...
  const filterOptions = [
    ...
    { key: "pagos", label: "Pagos", count: counts.pagos, icon: CheckCircle },
    {
      key: "vencidos",
      label: "Vencidos",
      ...
      <div className="p-4 md:p-6">
        <div className="mb-4 md:mb-6">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Consulte e acompanhe seus pagamentos
          </p>
        </div>
          ...
          <div className="mt-4 space-y-2 sm:hidden">
            {filterOptions.map(({ key, label, count, icon: Icon }) => (
              <button
                ...
                onClick={() => {
                  setFilter(
                    key as "todos" | "pendentes" | "pagos" | "vencidos"
                  );
        ...
        <div className="hidden sm:flex space-x-2 mt-6">
          {filterOptions.map(({ key, label, count, icon: Icon }) => (
            <button
              key={key}
              onClick={() =>
                setFilter(key as "todos" | "pendentes" | "pagos" | "vencidos")
              }
      ...
      {sortedPayments.length === 0 ? (
        <div className="text-center py-8 md:py-12">
          ...
          <h3 className="text-base md:text-lg font-medium text-gray-900 mb-2">
            ...
            {filter === "pagos" && "Nenhum pagamento realizado"}
            {filter === "vencidos" && "Nenhum pagamento vencido"}
          </h3>
          <p className="text-sm md:text-base text-gray-600 px-4">
            {filter === "todos" &&
              "Seus pagamentos aparecerão aqui quando houver consultas agendadas."}
            {filter === "pendentes" && "Pagamentos pendentes aparecerão aqui."}
            {filter === "pagos" &&
              "Seu histórico de pagamentos aparecerá aqui."}
            {filter === "vencidos" && "Pagamentos em atraso aparecerão aqui."}
          </p>
        </div>
      ) : (
        ...
        <div className="fixed bottom-4 left-4 right-4 sm:hidden bg-white border border-gray-200 rounded-lg shadow-lg p-3 z-20">
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">
              ...
              {filter === "pagos" && `${counts.pagos} pagos`}
