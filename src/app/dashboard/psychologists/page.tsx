...
interface Psychologist {
  ...
  endereco: string;
  numero: string;
  pacientes: number; // Apenas para contar
  atendimentos: number; // Apenas para contar
}
...
export default function PsychologistsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const doctors = (await api.get(`/psicologos`)) as Psychologist[];

      setPsychologists(doctors);
      setFilteredPsychologists(doctors);
      setLoading(false);
      setError(null);
    }
    fetchData();
  ...
  const totalPacientes = psychologists.reduce(
    (acc, p) => acc + (p.pacientes || 0),
    0
  );
  const totalAtendimentos = psychologists.reduce(
    (acc, p) => acc + (p.atendimentos || 0),
    0
  );
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  {filteredPsychologists.length === 0 ? (
                    ...
                    <>
                      {filteredPsychologists.length} resultado(s) para{" "}
                      <span className="font-medium">
                        &quot;{searchTerm}&quot;
                      </span>
                    </>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              Aguarde enquanto os psicólogos são carregados na plataforma.
            </p>
          </div>
        ) : filteredPsychologists.length > 0 ? (
